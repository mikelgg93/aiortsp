...
class RTSPMediaSession:
    ...
    async def setup(self):
        ...
        # Get SDP
        self.logger.info("Sending DESCRIBE request...")
        resp = await self._send("DESCRIBE", headers={"Accept": "application/sdp"})
        self.logger.info(f"Received DESCRIBE response: {resp.status}")
        ...
        if resp.status == 200:
            ...
            raise RTSPError(f"Failed to get SDP: {resp.status} {resp.msg}")
